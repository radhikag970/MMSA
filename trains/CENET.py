...
class CENET:
    ...
    def do_train(self, model, dataloader, return_epoch_results=False):
        ...
        optimizer_grouped_parameters = [
            {
                "params": [
                    ...
                    if not any(nd in n for nd in no_decay) and not any(nd in n for nd in CE_params)
                ],
            ...
            },
            {
                "params": [
                    p for n, p in param_optimizer
                    if any(nd in n for nd in no_decay) and not any(nd in n for nd in CE_params)
                ],
                "weight_decay": 0.0,
            },
        ]
        ...
        while True:
            ...
            logger.info(f"TRAIN-({self.args['model_name']}) ({epochs - best_epoch}/{epochs}) >> loss: {train_loss:.4f}")
            pred, true = torch.cat(y_pred), torch.cat(y_true)
            train_results = self.metrics(pred, true)
            logger.info('Multimodal: >> ' + dict_to_str(train_results))
    ...
    def do_test(self, model, dataloader, mode="VAL"):
        ...
        logger.info(f"{mode}-({self.args['model_name']}) >> loss: {eval_loss:.4f}")
        pred, true = torch.cat(y_pred), torch.cat(y_true)
        results = self.metrics(pred, true)
        logger.info('Multimodal: >> ' + dict_to_str(results))
